...
def calcular_fitness(individuo, dados_conteineres, max_peso, max_volume):
    ...
    if peso_total > max_peso:
        valor_total -= (peso_total - max_peso) * 1000  # Penalidade por excesso de peso
    if volume_total > max_volume:
        valor_total -= (volume_total - max_volume) * 100  # Penalidade por excesso de volume
    return max(0, valor_total)  # Fitness nunca é negativo
...
def mutacao(individuo, taxa_mutacao):
    for i in range(len(individuo.genoma)):
        if random.random() < taxa_mutacao:
            individuo.genoma[i] = 1 - individuo.genoma[i]  # Inversão de bit
...
def algoritmo_genetico(dados_conteineres, max_peso, max_volume, tamanho_populacao, num_geracoes, taxa_crossover, taxa_mutacao):
    ...
    for _ in range(num_geracoes):
        # Calcular o fitness de cada indivíduo na população
        ...
        nova_populacao = []
        # Criar nova população usando seleção por torneio e crossover
        while len(nova_populacao) < tamanho_populacao:
            if random.random() < taxa_crossover:
                ...
                nova_populacao.append(selecao_torneio(populacao))  # Caso não ocorra crossover, um indivíduo é selecionado
